...
/**
* Adding and deleting new topology break-points kernel
*/

public class TopDimChange {

    DCPSampler sampler;
    
    public TopDimChange (DCPSampler s){
	...
	if (sampler.set.unif.nextDouble() < 0.5){// keep current tree on the left
		...
		log_tree_proposal = -1.0*Math.log(sampler.TreeSpan - 1);
	    }
	    else{
		right_tree = sampler.ProposeEndTree(right_part.cTree, left_tree);
		log_tree_proposal = -1.0*Math.log(sampler.TreeSpan - 2);
	    }
	...
	double logRatio = pLogLikelihood - cLogLikelihood
	    ...
	    - Math.log(sampler.TreeSpan-1) // prior on topology configuration
	    - log_tree_proposal // new tree proposal probability
	    + sampler.logTopLambda;
	...
	if (enough_room){
		...
		double logRatio = pLogLikelihood - cLogLikelihood
		    ...
		    + 2*sampler.logTopLambda
		    - Math.log(sampler.topology_changes + 1)
		    - Math.log(sampler.TreeSpan-1) // prior on topology configuration after canceling with proposal
		    + Math.log(proposing_second_reciprocal)
		    - Math.log(sampler.lenSeq - sampler.topology_changes - 1);
	...
	if (sampler.set.unif.nextDouble() < 0.5){
		...
		if (right_change_part.cTree == after_collapse_tree){
		    ...
		    if (right_top_change == sampler.partition_list.size()){
			proposing_second_reciprocal = sampler.lenSeq - 
			    ((Partition)sampler.partition_list.elementAt(left_to_delete)).left.nuc_position - 1;
		    }
		    else{
			Partition very_right = (Partition)sampler.partition_list.elementAt(right_top_change);
			proposing_second_reciprocal = very_right.left.nuc_position - 
			    ((Partition)sampler.partition_list.elementAt(left_to_delete)).left.nuc_position - 1;
		    }
		}
	    }
		...
		if (left_change_part.cTree == after_collapse_tree){
		    ...
		    proposing_second_reciprocal = 
			((Partition)sampler.partition_list.elementAt(right_to_delete)).left.nuc_position 
			- very_left.left.nuc_position;
		}
	    }
	...
	if (!inconsistency){// delete one topology change point
	    ...
	    //System.out.println("log_tree_proposal: " + log_tree_proposal);
    ...
    /** 
     * Returns a randomly chosen topology break-point to remove
     */
    
    protected int ProposeTopChangePointToDelete(){
	
	int[] top_ch_index = new int[sampler.topology_changes - 1];
	...
	int rnd = (int)(sampler.set.unif.nextDouble()*(sampler.topology_changes - 1));
	
	
	return top_ch_index[rnd];
	
    }


    /** 
     * Returns a randomly chosen location of a new topology break-point
     */

    
    protected int ProposeTopChangePointToInsert(){
	...
	while (!good_point){
		...
		curr_part = (Partition)sampler.partition_list.elementAt(i);
		if (curr_part.IsTopologyChange() && new_point == curr_part.left.nuc_position)
		    good_point = false;
	    }
    ...
    /**
     * Returns the closest topology break-point to the right
     * of the reference change-point
     * @param landing reference change-point
     */

    protected int FindRightTopChange(int landing){
	Partition land_part = (Partition)sampler.partition_list.elementAt(landing);
	boolean par_change = false;
	int right_change;
